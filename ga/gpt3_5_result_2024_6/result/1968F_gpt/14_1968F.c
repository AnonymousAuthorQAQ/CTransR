...
fn sort(ii: &mut [usize], l: usize, r: usize, aa: &[u32]) {
    if l < r {
        ...
        let mut j = l;
        let mut k = r;
        let i_ = aa[l + (rand_() as usize) % (r - l)];
        let mut tmp;
...
fn main() {
    ...
    for _ in 0..t {
        ...
        for i in 1..=n {
            let mut input = String::new();
            std::io::stdin().read_line(&mut input).unwrap();
            aa[i] = input.trim().parse().unwrap() ^ aa[i - 1];
        }
        ...
        for _ in 0..q {
            ...
            let search_a = search(aa[r], l, &ii, &aa) + 1;
            let search_b = search(aa[l], r, &ii, &aa);

            if aa[l] == aa[r] || ii[search_a] < ii[search_b as usize] {
                println!("YES");
            } else {
                println!("NO");
            }

...
impl Queue {
    ...
    fn create_queue(max_elements: i32) -> Queue {
        Queue {
...
fn main() {
    ...
    let queue_length: i32 = input.trim().parse().expect("Invalid input or queue size is too small");
    ...
    if !q.is_full() {
        io::stdin().read_line(&mut enqueue_input).expect("Failed to read input");
        let enqueue_value: ElementType = enqueue_input.trim().parse().expect("Invalid input");

...
fn main() {
    ...
    let reader = input.lines();

    let queue_length: usize = reader.next().unwrap().unwrap().trim().parse().unwrap();
    ...
    if !Q.is_full() {
        let enqueue_value: i32 = reader.next().unwrap().unwrap().trim().parse().unwrap();

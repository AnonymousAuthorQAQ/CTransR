...
fn main() {
    ...
    let input_values: Vec<f64> = input.trim().split_whitespace().map(|x| x.parse().unwrap()).collect();
    let r_square = input_values[0];
    let p_square = input_values[1];
    ...
    io::stdin().read_line(&mut input).expect("Failed to read line");
    let input_values: Vec<f64> = input.trim().split_whitespace().map(|x| x.parse().unwrap()).collect();
    let r_cube = input_values[0];
    let p_cube = input_values[1];

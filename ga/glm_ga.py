...
from zhipuai.core._errors import APITimeoutError
from zhipuai import ZhipuAI
from dataclasses import dataclass
from time import sleep

api_key = ""
with open("../api/api_key_glm", "r") as f:
    api_key = f.read().splitlines()[0]

temperature_default = float(0.95)
top_default = float(0.7)
...
def process_file(input_content,temperature_value,top_p_value):
    ...
    max_retries = 6
    # print(temperature_value, top_p_value)
    # sleep(100)
    try:
        temperature_value = float(temperature_value)
        top_p_value = float(top_p_value)
    except ValueError:
        raise ValueError("temperature_value and top_p_value must be float")
...
def input_cargo_run(input_data, output):
    ...
    try:
        ...
        with open('input.txt', 'w') as f:
            f.write(input_data)
        result = subprocess.run('cargo run <input.txt', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=30)
        os.remove('input.txt')
        os.chdir('..')
        if result.returncode != 0:
            print(result.stderr.decode())
...
def local_search(individuals: list):
    for individual in individuals:
        ...
        if individual.temperature_direction == 0:
            individual.temperature = individual.temperature - 0.05
        else:
            individual.temperature = individual.temperature + 0.05
        if individual.temperature >= 0.99:
            individual.temperature = 0.99
        if individual.temperature <= 0.01:
            individual.temperature = 0.01
        if individual.top_p_direction == 0:
            individual.top_p = individual.top_p - 0.1
        else:
            individual.top_p = individual.top_p + 0.1
        if individual.top_p >= 0.9:
            individual.top_p = 0.9
        if individual.top_p <= 0.1:
            individual.top_p = 0.1
    return individuals

...
fn main() {
    ...
    for _ in 0..t {
        ...
        let ps1: usize = iter.next().unwrap().parse().unwrap() - 1; // Convert to 0-based index
        let ps2: usize = iter.next().unwrap().parse().unwrap() - 1; // Convert to 0-based index
        ...
        for (i, val) in input.split_whitespace().enumerate() {
            p[i] = val.parse::<usize>().unwrap() - 1; // Convert to 0-based index
        }

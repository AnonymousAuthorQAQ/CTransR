...
fn main() {
    ...
    for _ in 0..t {
        ...
        let params: Vec<usize> = input.split_whitespace()
                                       ...
                                       .collect();
        let n = params[0];
        let k = params[1] as i64;
        let ps1 = params[2] - 1; // Convert to zero-based index
        let ps2 = params[3] - 1; // Convert to zero-based index

        input.clear();
        stdin.read_line(&mut input).unwrap();
        let p: Vec<usize> = input.split_whitespace()
                                  .map(|x| x.parse::<usize>().unwrap() - 1) // Convert to zero-based index
                                  .collect();

...
fn main() {
...

    {
        ...
        if let Some(shared_ptr) = weak_ptr2.upgrade() {
            // Modify the value safely through the RefCell
            *shared_ptr.borrow_mut() = MyStruct::new(20);
    ...
    // Resetting `ptr1` will drop the Rc pointer and potentially destroy the object
    drop(ptr1);
    observe(&weak_ptr1);
}
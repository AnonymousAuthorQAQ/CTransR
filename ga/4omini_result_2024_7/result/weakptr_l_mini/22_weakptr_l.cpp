...
fn main() {
...

    {
        ...
        if let Some(shared_ptr) = weak_ptr2.upgrade() {
            shared_ptr.value = 20; // Change the value
    ...
    // Reset ptr1 by dropping it
    drop(ptr1);
    observe(&weak_ptr1); // Check if the object is still alive

    // Note: `ptr1` is dropped at the end of the main function,
    // triggering the destructor and the "Destructor" message.
}
...
impl AbstractK for K2D {
    ...
    fn is_inside(&self) -> bool {
        false // This should be conditional in derived structs
    }
}
...
fn main() {
    ...
    let r = nums[0].parse::<f64>().expect("Invalid radius");
    let p = nums[1].parse::<f64>().expect("Invalid parameter");
    ...
    input.clear();  // Clearing the input buffer for next read

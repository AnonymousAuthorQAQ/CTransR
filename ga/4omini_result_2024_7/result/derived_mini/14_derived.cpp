...
impl AbstractK for K2D {
    fn is_inside(&self) -> bool {
        false // This will be overridden by SquareK
    }
...
impl AbstractK for K3D {
    fn is_inside(&self) -> bool {
        false // This will be overridden by CubeK
    }
...
fn main() {
    ...
    let r_square: f64 = inputs[0].parse().expect("Invalid number");
    let p_square: f64 = inputs[1].parse().expect("Invalid number");
    ...
    io::stdin().read_line(&mut input).expect("Failed to read line");
    let inputs: Vec<&str> = input.trim().split_whitespace().collect();
    let r_cube: f64 = inputs[0].parse().expect("Invalid number");
    let p_cube: f64 = inputs[1].parse().expect("Invalid number");

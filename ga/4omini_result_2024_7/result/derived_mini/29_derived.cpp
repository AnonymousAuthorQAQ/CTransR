...
fn main() {
    ...
    let mut inputs: Vec<f64> = input.trim().split_whitespace().map(|s| s.parse().unwrap()).collect();
    let square = SquareK::new(inputs[0], inputs[1]);
    ...
    io::stdin().read_line(&mut input).unwrap();
    inputs = input.trim().split_whitespace().map(|s| s.parse().unwrap()).collect();
    let cube = CubeK::new(inputs[0], inputs[1]);

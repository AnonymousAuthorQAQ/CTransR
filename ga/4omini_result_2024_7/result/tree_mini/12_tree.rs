...
// Define the type alias for the element type
...
// Define the SearchTree type to be an optional Box<TreeNode>
...
fn insert(x: ElementType, t: SearchTree) -> SearchTree {
    match t {
        ...
        Some(mut node) => {
            ...
            Some(node) // Return the node (Box<TreeNode>) back
        }
    }
}
        ...
        Some(mut node) => {
            if x < node.element {
                ...
                if node.left.is_some() && node.right.is_some() {
                    ...
                    let tmp_node = node.clone(); // Clone to retain ownership
                    if node.left.is_none() {
                        return node.right;
                    } else {
                        return node.left;
...
fn retrieve(node: &Option<Box<TreeNode>>) -> ElementType {

        ...
        match x.cmp(&node.element) {
            ...
            Ordering::Equal => {
                if let Some(mut right) = node.right.take() {
                    node.element = find_min(&right).unwrap().as_ref().element;
                    node.right = right;
                } else if let Some(mut left) = node.left.take() {
                    node.element = find_max(&left).unwrap().as_ref().element;
                    node.left = left;
                } else {
                    return;
                }
...
fn main() {
    ...
    println!("Min is {}, Max is {}", retrieve(find_min(&tree).unwrap().as_ref()), retrieve(find_max(&tree).unwrap().as_ref()));
}